...
- **Statistics tab**: overview of the manga library. When more than one library exists, a row of pill buttons (**All Libraries** plus one per library) appears above the tiles — selecting a library re-fetches `GET /api/stats?library_id=N` so every tile and ranked list switches to that scope. With a single library the switcher is hidden. Contents:
  ...
  - *Favorite Genres* — top 10 genres weighted by **reading history**, where a **volume counts as 4 chapters** and a chapter (or a file labeled as both volume and chapter, or with only a bare number) counts as 1; volume-vs-chapter comes from the scanned `chapters.volume` / `chapters.number` labels. Each read item contributes its weight to every genre tagged on that chapter's manga, so a reader who has finished 40 chapters of a 3-genre series adds 40 to each of those three genres, and one drama volume adds 4 to Drama. Empty-state copy prompts the user to read some chapters when no progress exists. This same volume-weighted ranking drives the Home page's Discover and per-genre ribbons (server-side, shared computation), so the two views agree. The three ranked lists use a `repeat(auto-fit, minmax(300px, 1fr))` grid so wide screens get 3 columns, typical laptops get 2, and mobile collapses to 1.
...
- **Database tab**: maintenance operations for the server's database and on-disk cache:
  - *CBZ Cache* — displays the current cache size alongside the configured cap (e.g. `1.4 GB / 20.0 GB`). **Clear Cache** deletes every extracted chapter directory in `CBZ_CACHE_DIR` (pages are re-extracted on next access). Below the current-size row, an expanded block exposes:
    ...
    - *Auto-clear schedule* — a **cadence** segmented control (Off / Daily / Weekly). **Weekly** reveals a day-of-week `<select>`; both Daily and Weekly reveal a `<input type="time">` (server local time) and a "Clear chapters not read in N days" number input. The scheduled clear is always an **age sweep** — it drops only chapters not read within the window and never interrupts a chapter being actively read (only the manual **Clear Cache** button wipes everything). A "Next auto-clear: …" status line shows the concrete next fire. Save fires `PUT /api/admin/cbz-cache-settings` and refreshes the displayed next-run timestamp. See [api.md § CBZ Cache Settings](./api.md#cbz-cache-settings) and [scanner.md § Auto-Clear Scheduler](./scanner.md#auto-clear-scheduler).
