...
Returns `{ chapter, chapterEnd, volume, volumeEnd }` — all `float|null`.

**A single file or folder can span a RANGE of chapters and/or volumes** (e.g.
`Yamada-kun and the Seven Witches v17-18.cbz` holds volumes 17–18 in one CBZ).
When a range is detected, `volume`/`chapter` hold the **start** and
`volumeEnd`/`chapterEnd` the **inclusive end**; for a single value the `*End`
fields are `null`. The start is the sort/index anchor everywhere, so every
existing query keeps working — the `*End` columns are purely additive.

### Volume patterns recognized

| Pattern | Example | Parses as |
|---|---|---|
| `Vol.N`, `Vol N`, `Volume N`, `Vols N` | `Vol.03`, `Volume 2` | volume start |
| `vN`, `v.N` | `v01`, `V2` | volume start |
| **Range** `Vol N-M`, `vN-M`, `vN-vM` | `Vol. 17-18`, `v17-v18` | volume `17`, volumeEnd `18` |

### Chapter patterns recognized

| Pattern | Example | Parses as |
|---|---|---|
| `Ch.N`, `Ch N`, `Chapter N`, `Chs N` | `Ch.023.5`, `Chapter 5` | chapter start |
| `cN`, `c.N` | `c01`, `C01` | chapter start |
| **Range** `Ch N-M`, `cN-M`, `cN-cM` | `Ch. 10-12`, `c001-005` | chapter `10`, chapterEnd `12` |

### Range separators

`-`, `–` (en-dash), `—` (em-dash), `~` (Japanese range), the word `to`, and the
**discrete joiners** `&` `+` `,` (one file holding multiple numbers, e.g.
`v17 & 18`). A **chained** run is consumed whole — `Vol 1,2,3,4,5` →
`volume 1`, `volumeEnd 5`; the first and last numbers become the span. (This
matters: if only the first pair were taken, the leftover numbers would leak into
the *other* axis via the bare-number fallback — `Vol 1,2,3` would mis-parse as
volume 1-2 **plus** a phantom chapter 3.) A run mixing axes stops at the second
prefix, so `Vol 1-2-3 Ch 5-12` still yields volume 1-3 **and** chapter 5-12. A
range is accepted only when it is **ascending** and the span is within a sanity
cap (`MAX_VOLUME_SPAN = 50`, `MAX_CHAPTER_SPAN = 100`) — a descending or oversized
"range" (`c18-17`, `c1-9999`) falls back to the single start value.

### Fallback
If no explicit `Ch`/`Vol` prefix is found, a bare numeric range (`001-005`) is
tried first — rejecting year–year spans (`2017-2018`) — then the first
standalone number that is not a 4-digit year (1900–2099) is used as the chapter.

### Pre-processing
Before matching, the parser:
1. Strips known archive/image extensions
2. Removes bracketed groups (`[Fansub]`, `[HQ]`)
3. Normalizes `_` to spaces — but **deliberately keeps `&` `+` `,`** so the range
   patterns can consume them as discrete joiners (the pre-range version of this
   step collapsed them to spaces, which lost `v17 & 18`).

### Examples

| Input | volume | volumeEnd | chapter | chapterEnd |
|---|---|---|---|---|
| `Vol. 03 Ch. 023.5 - Title [Group]` | 3 | null | 23.5 | null |
| `Yamada-kun ... v17-18.cbz` | 17 | 18 | null | null |
| `Vol. 1-2 Ch. 5-12` | 1 | 2 | 5 | 12 |
| `v17 & 18` | 17 | 18 | null | null |
| `Chapter 10-12` | null | null | 10 | 12 |
| `001-005` | null | null | 1 | 5 |
| `2017-2018` (years) | null | null | null | null |
| `Vol 1.cbz` | 1 | null | null | null |
| `c01` | null | null | 1 | null |

Regression matrix: [server/test/chapterParser.test.js](../server/test/chapterParser.test.js).
